...
public class LibroController {
	...
	public String findOne(@RequestParam("idLibro") @Nullable Integer idLibro, 
		...
		modelMap.addAttribute("categroias",categoriaDAO.findAll());
	...
	public String add(@RequestParam("idLibro")@Nullable Integer idLibro,
					  ...
					  @RequestParam("tipoPasta") @Nullable String tipodePasta, 
			          ...
			          @RequestParam("idAautor") @Nullable Integer idAutor
			         
			          ) {
		
		if(idLibro == null) {
			Libro libro = new Libro(0,titulo,editorial,numPaginas,edicion,idioma,fechaPublicacion,descripcion,tipodePasta,
					isbn,numEjemplares,portada,presentacion,precio,
					categoriaDAO.findOne(idCategoria),
					autorDAO.findOne(idAutor));
					libroDAO.add(libro);
		}else {
			Libro libro2 = new Libro(idLibro,titulo,editorial,numPaginas,edicion,idioma,fechaPublicacion,descripcion,tipodePasta,
					isbn,numEjemplares,portada,presentacion,precio,
					categoriaDAO.findOne(idCategoria),
					autorDAO.findOne(idAutor)); 
					libroDAO.up(libro2);
